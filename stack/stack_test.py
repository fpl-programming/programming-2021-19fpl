...
class StackTestCase(unittest.TestCase):
    ...
    def test_my_new_stack_from_dict(self):
        """
        Create Stack out of python dict.
        Test that Stack can be created out ou python dict
        """
        stack_empty_dict = Stack({})
        self.assertEqual(stack_empty_dict.data, [])
        test_dict = {'key1': 'value1', 'key2': 'value2'}
        stack_dict = Stack(test_dict)
        self.assertEqual(stack_dict.data, list(test_dict))

    def test_my_new_stack_from_string(self):
        """
        Create Stack out of python string.
        Test that Stack can be created out ou python string
        """
        stack_empty_string = Stack('')
        self.assertEqual(stack_empty_string.data, [])
        test_string = 'Programming is fun'
        stack_string = Stack(test_string)
        self.assertEqual(stack_string.data, list(test_string))
